...
# ═══════════════════════════════════════════════════════════
# Task: cleanup_expired_quotes_task
# ═══════════════════════════════════════════════════════════
...
def cleanup_expired_quotes_task(self) -> dict:
    ...
    Quotations older than 30 days in ``'quoted'`` status are moved to
    ``'expired'`` status.

    Implementation:
      1. Open sync DB session.
      2. Find quotations where:
         - ``status == 'quoted'``
         - ``created_at < now() - interval '30 days'``
      3. Update found rows to ``status = 'expired'``.
      4. Log count of expired quotations.
    """
    ...
    try:
        ...
        # Using raw SQL for the Quotation table (from output module)
        result = db.execute(
            text("""
                UPDATE quotations
                SET status = 'expired',
        ...
        logger.info(
            "Expired quote cleanup completed",
            extra={"expired_count": affected_count, "cutoff": cutoff.isoformat()},
        )
        return {
            "status": "success",
            "task": "cleanup-expired-quotes",
            "expired_count": affected_count,
        }
