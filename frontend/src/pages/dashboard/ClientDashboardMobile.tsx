...
// Reference: handoff-designs/importer-home-mobile.html

const formatPrice = (price: number | null) => (price == null ? "—" : `¥${price.toLocaleString()}`);

// Same RFQ→StatusPill mapping as the desktop build.
...
export function ClientDashboardMobile() {
  ...
  return (
    <div className="flex flex-col gap-4" dir="rtl">
      {/* Welcome */}
      <div>
        <h1 className="text-[18px] font-bold text-slate-900">مرحباً، {user?.full_name || "مستورد"}</h1>
      </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] font-medium text-importer-600 transition-colors duration-150 hover:text-importer-700"
          >
            ...
            {catalogItems.slice(0, 3).map((p) => (
              <div key={p.id}>
                <ReelTile
                  ...
                  onClick={() => navigate(ROUTES.CATALOG.MARKETPLACE)}
                />
                <p className="mt-1.5 truncate text-center text-[10px] text-slate-500">
            ...
            {recentRfqs.map((rfq) => {
              ...
              return (
                <div
                  ...
                  className="flex cursor-pointer items-center justify-between border-b border-slate-100 py-3 last:border-b-0 transition-colors duration-150 active:scale-[0.98]"
                >
                  <StatusPill status={status} role="client" />
                  <div className="text-end">
                    <div className="text-[12px] font-semibold text-slate-700">{firstProductLine(rfq)}</div>
