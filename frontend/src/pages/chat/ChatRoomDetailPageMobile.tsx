...
// Full-screen conversation window (T8.6). Same shared hook/logic as
// ChatRoomDetailPageDesktop — only the container width/spacing differs.
// TopBar/BottomNav/Drawer come from the role's own layout.
export function ChatRoomDetailPageMobile() {
  const {
    room,
    messages,
    loading,
    ...
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-3xl flex-col p-4">
      <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-lg font-semibold text-slate-900">{otherName}</h2>
          {room?.rfq_id && <p className="text-xs text-slate-400">RFQ: {room.rfq_id.slice(0, 8)}...</p>}
        </div>
        {canSendQuote && (
          <button
            onClick={handleSendQuote}
            className="flex shrink-0 items-center gap-1.5 rounded-lg bg-supplier-500 px-3 py-2 text-xs font-medium text-white transition-colors duration-150 hover:bg-supplier-600 active:scale-[0.98]"
          >
