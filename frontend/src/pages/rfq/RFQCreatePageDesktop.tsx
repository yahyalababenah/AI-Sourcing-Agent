...
// Structured client-facing RFQ form (T8.1): product name/specs/quantity/
// optional images on the wide (right, RTL-first) column, a sticky live
// estimate preview on the narrow (left) column — matches the
// PricingCalcPageDesktop grid convention. Agents/admins still get the
// free-text LegacyRFQCreateForm (see RFQCreatePage.tsx).
export function RFQCreatePageDesktop() {
  const {
    productName,
    setProductName,
    specifications,
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex items-center gap-4">
        <button
          ...
          className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-600 transition-colors duration-150 hover:bg-slate-50 active:scale-[0.98]"
        >
          → العودة
        </button>
        <div>
      ...
      {/* noValidate: this form has its own Arabic validation messages —
          native constraint validation would intercept submit first. */}
      <form onSubmit={handleSubmit} noValidate className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_380px]">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                dir="ltr"
              />
              <p className="mt-1 text-xs text-slate-400">اختياري — يفعّل معاينة التكلفة التقديرية جانبه</p>
            </div>
          </div>
